...
# ---------- nginx: /api/-Block vor Regex-locations schuetzen (^~) ----------
# Bild-URLs wie /api/uploads/speakers/x.jpg muessen IMMER an die API gehen.
# '^~ /api/' verhindert, dass eine evtl. Regex-location (z.B. fuer Dateiendungen)
# /api/* abfaengt. Idempotent, mit nginx -t-Absicherung und Rollback.
step "nginx: /api/-Block mit ^~ absichern"
APIPFX_CONF=$(grep -rlsE "location[[:space:]]+\^?~?[[:space:]]*/api/" /etc/nginx 2>/dev/null \
  | grep -vE '\.(bak|orig|save|dpkg-[a-z]+)' | head -1 || true)
if [ -z "${APIPFX_CONF:-}" ]; then
  warn "keine nginx-Datei mit 'location /api/' gefunden — uebersprungen"
elif grep -qE "location[[:space:]]+/api/[[:space:]]*\{" "$APIPFX_CONF" \
     && ! grep -qE "location[[:space:]]+\^~[[:space:]]*/api/" "$APIPFX_CONF"; then
  cp "$APIPFX_CONF" "${APIPFX_CONF}.bak.apiprefix.$TS"
  sed -i -E "s#location([[:space:]]+)/api/#location\1^~ /api/#" "$APIPFX_CONF"
  if nginx -t >/dev/null 2>&1; then
    ok "location ^~ /api/ gesetzt in $APIPFX_CONF"
  else
    cp "${APIPFX_CONF}.bak.apiprefix.$TS" "$APIPFX_CONF"
    warn "nginx -t nach ^~-Aenderung fehlgeschlagen — zurueckgerollt"
  fi
else
  ok "location /api/ bereits mit ^~ abgesichert"
fi
